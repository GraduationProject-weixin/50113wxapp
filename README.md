...
第7章结论。总结论文的主要工作，提出进一步完善基于微信小程序的校园二手数码交易平台的目标和明确方向。
...
# 第3章  系统分析
## 3.1 可行性分析
需要使用大部分精力开发的基于微信小程序的校园二手数码交易平台为了充分降低开发风险，特意在开发之前进行可行性分析这个验证系统开发是否可行的步骤。本文就会从技术角度，经济角度，还有操作角度等进行综合阐述。
### 3.1.1技术可行性
本校园二手数码交易平台采用Java技术，基于微信小程序，采用MYSQL数据库进行开发的。
